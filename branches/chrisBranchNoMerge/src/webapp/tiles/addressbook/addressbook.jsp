<%@page contentType="text/html" import="edu.ufl.osg.webmail.Constants,
                                        ...
                                        edu.ufl.osg.webmail.data.MailingEntry,
                                        edu.ufl.osg.webmail.data.AddressList,
                                        edu.ufl.osg.webmail.data.MailingList,
...
<%
	MailingList mailingList = (MailingList)pageContext.getAttribute("mailingList");
	if(request.getParameter("mailing") != null) {
		int curMailing = mailingList.size();
		//MailingEntry curMailing = (MailingEntry) mailingList.get(Integer.valueOf(request.getParameter("mailing")).intValue());
		//pageContext.setAttribute("curMailing", curMailing);
	}
%>
   		...
   		<table class='abkLayout'>
			...
			<td>
				<table class="headerTable" id="mailingList" cellspacing="0" cellpadding="4" width="100%">
					   ...
					   <tr class="header">
					      <th><bean:message key="addressbook.name"/></th>
					      <th><bean:message key="addressbook.numContacts"/></th>
					      <th></th>
					   </tr>
					   <%
					   		Map reloadParams = new HashMap();
					   		pageContext.setAttribute("reloadParams", reloadParams);
					   %>
				   		<c:forEach items="${mailingList}" var="mailingEntry" varStatus="index">
				    		<%
				    			LoopTagStatus index = (LoopTagStatus)pageContext.getAttribute("index");
				    			reloadParams.put("mailing", index.getIndex());
				    		%>
				    <c:choose>
				     <c:when test="${index.index % 2 == 0}">
				      <tr>
				     ...
				     <td>
				     	<html:link forward="addressbook" name="reloadParams" scope="page">
